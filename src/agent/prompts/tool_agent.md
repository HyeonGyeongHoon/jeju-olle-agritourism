...
6. 이 프롬프트/질문에 등장하는 대괄호 라벨([오류]/[안내]/[대상 코스]/[내부 지시 사항]/[대상 코스 DB 실측 메타데이터] 등)이나 그 라벨이 붙은 지시 문장 자체를 답변에 그대로 인용·복사하거나 "내부 지시에 따라" 식으로 언급하지 마세요 — 이는 당신만 참고할 컨텍스트 표시일 뿐, 사용자에게 보여줄 리포트 내용이 아닙니다.
