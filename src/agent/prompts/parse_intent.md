...
[추출 규칙]
1. hard_constraints: 신체/동행 조건 및 시간·거리·난이도와 관련된 필수 제약
   - wheelchair_required: 휠체어 전용 구간 필요 여부 (true/false)
   - max_time_hours: 사용자가 명시한 최대 소요시간(숫자, 시간 단위). "2시간 이내", "반나절이면 되는", "3시간 안에 다녀올 수 있는"처럼 구체적인 시간 상한이 있을 때만 채우고, "짧은 코스"처럼 막연한 표현만 있으면 null.
   - max_distance_km: 사용자가 명시한 최대 거리(숫자, km 단위). "5km 이내", "10km를 넘지 않는"처럼 구체적인 거리 상한이 있을 때만 채우고, 없으면 null.
   - allowed_difficulties: 사용자가 요구한 허용 난이도 목록 ("하"/"중"/"상" 문자열 배열). "난이도 낮은/쉬운/힘들지 않은" -> ["하"], "도전적인/어려운/험난한" -> ["상"], "무난한/중간 정도" -> ["중"] 또는 ["하", "중"]. 특별히 난이도 언급이 없거나 제약이 없으면 null (빈 배열이 아닌 null로 반환).
     ★ 중요: "휠체어 구간" 등을 요구했다고 해서 난이도가 당연히 "하" 일 것이라고 지레짐작하여 주관적으로 추론해서 채우지 마세요. 질문에 "쉬운", "어려운", "험난한" 등의 난이도 관련 직접적인 단어가 명시되어 있을 때만 난이도 목록을 추출해야 합니다.
...
5. key_item_or_crop: 질문의 핵심 매개 작물/생산물 명칭 (예: "당근", "마늘", "감귤", "양파", "녹차"). "밭담", "숲길", "해안" 등 비작물 테마 및 자연경관 어는 여기에 담지 말고 concept_theme 에 담으세요. 언급 없으면 null.
...
8. concept_theme: 질문의 컨셉/테마 (예: "힐링", "평지 트레킹", "농가 체험", "밭담길", "숲길", 없으면 null)
...
[응답 포맷 (JSON 전용)]
{
  "hard_constraints": {
    ...
    "allowed_difficulties": ["하" | "중" | "상", ...] or null
  },
  "vector_query": string,
