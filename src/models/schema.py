...
class CourseSchema(BaseModel):
    ...
    estimated_time_hours: float = Field(..., ge=0.0, description="예상 소요 시간 (시간)")
...
class WheelchairSegmentSchema(BaseModel):
    ...
    segment_name: str = Field(..., description="휠체어 구간 명칭 (예: 종달리 옛 소금밭 ~ 성산갑문 입구 구간)")
    start_address: str = Field(..., description="휠체어 구간 시작점 지번/도로명 주소")
    distance_km: float = Field(..., ge=0.0, description="구간 거리 (km)")
    difficulty_level: Literal["상", "중", "하"] = Field(..., description="난이도 등급 (상, 중, 하 중 택일)")

class SearchRequest(BaseModel):
    """하이브리드 RAG 검색 API (POST /api/v1/search) 요청 검증을 위한 모델입니다."""
    query: str = Field(..., min_length=1, description="사용자의 자연어 챗봇 질문")
    session_id: str = Field(..., min_length=1, description="멀티턴 대화 유지용 세션 식별자 (필수)")
    top_k: int = Field(default=3, ge=1, le=10, description="검색할 임베딩 청크 최대 개수")
...
class SearchResponse(BaseModel):
    ...
    answer_context: str = Field(..., description="최종 조립된 답변 텍스트 또는 프롬프트용 컨텍스트")
    sources: list[WheelchairSourceSchema] = Field(default=[], description="답변에 참조된 원천 휠체어 데이터 목록")
