...
def test_route_after_rewrite_sends_all_non_recommendation_intents_to_quick_response():
    ...
    other(당시엔 course_info/olle_general_info 도 별도 값이었음)는 (quick_responder 로
    들어갔었음에도) retriever 로 잘못 돌려보냈습니다(2026-07-24 발견 및 수정 —
    route_after_location_resolve 와 동일하게 "course_recommendation 이 아니면
    quick_response" 기준으로 통일). 2026-07-26: course_info/olle_general_info 는
    info_lookup 으로 통합됨."""
    for category in (
        IntentCategory.OTHER,
        IntentCategory.INFO_LOOKUP,
