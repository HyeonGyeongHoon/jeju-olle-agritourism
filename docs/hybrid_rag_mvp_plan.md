...
- **에이전트 프롬프트 워크플로우**: 사용자 의도 파싱(Parser), 하이브리드 RAG 및 계층적 완화(Fallback), 맞춤 도슨트 답변 생성을 분리한 3단계 프롬프트 체인([agent_prompt_workflow.md](./docs/agent_prompt_workflow.md))을 적용합니다.
...
CREATE TABLE course_chunks (
    ...
    embedding VECTOR(4096), -- Upstage Solar Embedding 기준 4096 차원
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
- **다중 제약 조건 불일치 시 계층적 완화(Fallback) 파이프라인**:
  - *상황*: 코스 수의 제한성(총 27개 내외)으로 인해 사용자의 다중 요구 조건(예: 휠체어 접근 + 1시간 이내 + 난이도 하)을 100% 만족하는 완벽 매칭 코스가 존재하지 않아 빈 결과(Empty Result)가 발생하는 경우
  - *대응*: 절대적 제약 조건(예: 휠체어 가능 여부)은 엄격 유지하되 완화 가능한 제약 조건(소요시간, 난이도)의 범위를 단계적으로 넓히는 계층적 완화(Relaxed Search)를 수행합니다. 메타데이터 차이와 pgvector 유사도를 통합한 하이브리드 점수(Hybrid Ranking Score)로 상위 대안 코스를 탐색하고, 완화 이유와 함께 맞춤 브리핑을 제공합니다.
- **휠체어 구간 인포그래픽의 변형 패턴**:
